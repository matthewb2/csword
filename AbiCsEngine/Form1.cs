...
namespace AbiCsEngine
{
    public partial class Form1 : Form
    {
        public Form1()
        {
            ...
            AbiWordViewer viewer = new AbiWordViewer();
            viewer.Dock = DockStyle.Fill;
            this.Controls.Add(viewer);

            // 가상 문서 데이터 빌드
            // 명확하게 우리가 만든 Core의 Document임을 컴파일러에게 지정
            AbiCsEngine.Core.Document doc = new AbiCsEngine.Core.Document();

            // 타이틀 문단
            Paragraph pTitle = new Paragraph();
            pTitle.Runs.Add(new TextRun { Text = "AbiWord C# 레이아웃 명세서 문서", FontSize = 24, FontStyle = FontStyle.Bold, ForeColor = Color.DarkSlateBlue });
